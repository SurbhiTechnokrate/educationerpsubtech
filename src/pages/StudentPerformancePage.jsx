    ...
    <div className="space-y-5 sm:space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight">Academic Performance Analytics</h2>
        ...
        <div className="flex items-center gap-2 bg-white p-2 rounded-2xl border border-slate-200 shadow-sm self-start sm:self-auto">
          <GraduationCap className="w-4 h-4 text-blue-600 ml-1 flex-shrink-0" />
          <select
            value={selectedClass}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm">
          <span className="text-[10px] sm:text-xs font-semibold text-slate-400 block uppercase">Class Average Score</span>
          ...
          <span className="text-[10px] sm:text-[11px] text-slate-400 mt-1 block">Based on evaluations</span>
        </div>

        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm">
          <span className="text-[10px] sm:text-xs font-semibold text-slate-400 block uppercase">Pass Percentage</span>
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-2xl sm:text-3xl font-extrabold text-emerald-600">100%</span>
            <span className="text-[11px] font-semibold text-slate-500">{filteredStudents.length}/{filteredStudents.length}</span>
          </div>
          <span className="text-[10px] sm:text-[11px] text-slate-400 mt-1 block">Criteria: ≥ 35%</span>
        </div>

        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm">
          <span className="text-[10px] sm:text-xs font-semibold text-slate-400 block uppercase">Distinction (A+ / A)</span>
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-2xl sm:text-3xl font-extrabold text-blue-600">{distinctionPct}%</span>
            <span className="text-[11px] font-semibold text-slate-500">{distinctionCount} Students</span>
          </div>
          <span className="text-[10px] sm:text-[11px] text-slate-400 mt-1 block">Score ≥ 85%</span>
        </div>

        <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm">
          <span className="text-[10px] sm:text-xs font-semibold text-slate-400 block uppercase">Students Under Radar</span>
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-2xl sm:text-3xl font-extrabold text-amber-500">{atRisk.length}</span>
            <span className="text-xs font-semibold text-amber-600">Needs Focus</span>
          </div>
          <span className="text-[10px] sm:text-[11px] text-slate-400 mt-1 block">Attendance or Score risk</span>
        </div>
      </div>
          ...
          <div className="space-y-3">
            {toppers.length > 0 ? (
              toppers.map((stu, rank) => (
                <div
                  key={stu.id}
                  className="p-3 sm:p-3.5 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-between hover:bg-blue-50/50 transition-colors cursor-pointer gap-2"
                  onClick={() => openModal('studentProfile', stu)}
                >
                  <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                    <span className={`w-6 h-6 sm:w-7 sm:h-7 rounded-xl flex items-center justify-center font-black text-xs flex-shrink-0 ${
                      rank === 0
                    ...
                    <div className="min-w-0">
                      ...
                      <span className="text-[10px] sm:text-[11px] text-slate-400 block truncate">
                        {stu.class} - {stu.division} • Roll #{stu.rollNo}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <span className="block text-[9px] sm:text-[10px] font-bold text-slate-400">Aggregate</span>
                  </div>
                </div>
              ))
            ) : (
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-6 border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-4 gap-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <h3 className="font-bold text-base text-slate-900 truncate">Students Requiring Attention</h3>
            </div>
            <span className="px-2 py-0.5 bg-rose-50 text-rose-700 rounded-md text-[10px] font-bold flex-shrink-0">
              AI Risk Alert
            </span>
          </div>
          ...
          <div className="space-y-3">
            {atRisk.length > 0 ? (
              atRisk.map((stu) => (
                <div
                  key={stu.id}
                  className="p-3 sm:p-3.5 rounded-2xl bg-rose-50/40 border border-rose-100 flex items-center justify-between cursor-pointer gap-2"
                  onClick={() => openModal('studentProfile', stu)}
                >
                  <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                    <img
                      src={stu.photo}
                      alt={stu.name}
                      className="w-9 h-9 sm:w-10 sm:h-10 rounded-full object-cover border border-rose-200 flex-shrink-0"
                    />
                    <div className="min-w-0">
                      <h4 className="font-bold text-xs text-slate-900 truncate">{stu.name}</h4>
                      <span className="text-[10px] sm:text-[11px] text-slate-400 block truncate">
                        {stu.class} - {stu.division} • Att: <strong className="text-rose-600">{stu.attendance}</strong>
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <span className="block text-[9px] sm:text-[10px] font-semibold text-rose-600">Remedial</span>
                  </div>
                </div>
              ))
            ) : (
