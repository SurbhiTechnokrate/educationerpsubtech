      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight">Assignments & Homework</h2>
          ...
          return (
            ...
            >
              <div>
                ...
                <h3 className="text-sm sm:text-base font-bold text-slate-900 mt-3">{asn.title}</h3>
                <p className="text-xs text-slate-500 mt-1 line-clamp-2">{asn.description}</p>

                {/* Progress bar */}
                <div className="mt-4 p-3 sm:p-3.5 bg-slate-50 rounded-2xl border border-slate-100">
                ...
                <div className="flex flex-wrap items-center justify-between gap-1 text-[11px] sm:text-xs text-slate-400 font-medium mt-3">
                  <span>Assigned: {asn.assignedDate}</span>
                  <span className="text-rose-600 font-semibold">Due: {asn.dueDate}</span>
                  <span>Max: {asn.maxMarks}</span>
                </div>
              </div>

              <div className="mt-4 sm:mt-5 pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2">
                <button
                  onClick={() => addToast(`Reviewing submissions for ${asn.title}`, 'info')}
                  className="px-3.5 sm:px-4 py-2 bg-slate-100 hover:bg-blue-50 hover:text-blue-600 text-slate-700 rounded-xl text-xs font-bold transition-colors"
                >
                  Review ({asn.submittedCount})
                </button>
                <button
